...
public class BillableImpl implements Billabledao {
	...
	public List<Billable> getBillable() {	
		...
		String jpql="select count(*), month(deployementDate) from Billable group by month (deployementDate)";
		//		String jpql ="SELECT COUNT(u.opening_date) AS total, m.month" + //				"     from (" + //				"           SELECT 'Jan' AS MONTH" + //				"           UNION SELECT 'Feb' AS MONTH" + //				"           UNION SELECT 'Mar' AS MONTH" + //				"           UNION SELECT 'Apr' AS MONTH" + //				"           UNION SELECT 'May' AS MONTH" + //				"           UNION SELECT 'Jun' AS MONTH" + //				"           UNION SELECT 'Jul' AS MONTH" + //				"           UNION SELECT 'Aug' AS MONTH" + //				"           UNION SELECT 'Sep' AS MONTH" + //				"           UNION SELECT 'Oct' AS MONTH" + //				"           UNION SELECT 'Nov' AS MONTH" + //				"           UNION SELECT 'Dec' AS MONTH" + //				"          ) AS m" + //				"LEFT JOIN add_requirements u " + //				"ON MONTH(STR_TO_DATE(CONCAT(m.month, ' 2013'),'%M %Y')) = MONTH(u.opening_date)" + //				"GROUP BY m.month" + //				"ORDER BY 1+1";		
		Query query = (Query) manager.createQuery(jpql);	
		List<Billable> list = query.getResultList();	
		System.out.println("Guru::::"+list);
		if(list==null) {		
			return null;	
			}
		return list;
